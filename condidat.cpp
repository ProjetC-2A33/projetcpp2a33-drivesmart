...
void Condidat::on_btn_ajout_clicked()
{
    ...
    if (!digitsOnly.match(cinStr).hasMatch()) { QMessageBox::warning(this, "Erreur", "CIN doit contenir exactement 8 chiffres."); return; }
    ...
    if (nom.isEmpty() || !namePattern.match(nom).hasMatch() || nom.contains(QRegularExpression("\\d"))) { QMessageBox::warning(this, "Erreur", "Nom ne doit pas contenir de chiffres et ne peut pas être vide."); return; }
    if (prenom.isEmpty() || !namePattern.match(prenom).hasMatch() || prenom.contains(QRegularExpression("\\d"))) { QMessageBox::warning(this, "Erreur", "Prénom ne doit pas contenir de chiffres et ne peut pas être vide."); return; }
    QString sexe = ui->homme->isChecked() ? "H" : (ui->femme->isChecked() ? "F" : "");
    if (sexe.isEmpty()) { QMessageBox::warning(this, "Erreur", "Veuillez sélectionner le sexe."); return; }
    ...
    if (!digitsOnly.match(telStr).hasMatch()) { QMessageBox::warning(this, "Erreur", "Téléphone doit contenir exactement 8 chiffres."); return; }
    int tel = telStr.toInt(&ok);
    if (!ok) { QMessageBox::warning(this, "Erreur", "Téléphone invalide."); return; }
    QString type = ui->type->currentText();
    if (type.trimmed().isEmpty()) { QMessageBox::warning(this, "Erreur", "Veuillez sélectionner le type de permis."); return; }
