...
{
    // Définir le chemin du fichier dans le dossier Downloads
    QString downloadsPath = "C:/Users/rihab/Downloads";
    QDir dir;

    // Vérifier si le dossier existe, sinon le créer
    if (!dir.exists(downloadsPath)) {
        if (!dir.mkpath(downloadsPath)) {
            QMessageBox::warning(this, "Erreur", "Impossible de créer le dossier Downloads. Le fichier sera enregistré dans le répertoire courant.");
            downloadsPath = QDir::currentPath();
        }
    }

    // Construire le chemin complet du fichier
    QString fileName = downloadsPath + "/vehicules_export.pdf";
    ...
    if (!painter.begin(&printer)) {
        QMessageBox::critical(this, "Erreur", "Impossible d'initialiser l'impression PDF.");
        return;
    }
    ...
    painter.drawText(margin, yPos, pageWidth - 2 * margin, 30, Qt::AlignCenter, "Liste des véhicules");
    yPos += 60;

    // Dessiner les en-têtes de colonnes
    painter.setFont(headerFont);
    int colWidths[] = {100, 100, 100, 100, 100, 120, 120};
    QStringList headers;
    headers << "Modele" << "Matricule" << "Kilometrage" << "disponibilité"
            << "type d'energie" << "Etat de maintenance" << "date de maintenance";
    ...
    for (int col = 0; col < headers.size(); ++col) {
        ...
        painter.drawText(xPos + 5, yPos, colWidths[col] - 10, headerHeight,
    ...
    while (query.next()) {
        ...
        QString dateMaintenance = query.value(6).toDate().toString("dd/MM/yyyy");

        QStringList cellTexts;
        cellTexts << modele << matricule << kilometrage << disponibilite
                  << typeEnergie << etatMaintenance << dateMaintenance;

        for (int col = 0; col < headers.size(); ++col) {
            QString cellText = col < cellTexts.size() ? cellTexts[col] : "";

            painter.drawRect(xPos, yPos, colWidths[col], rowHeight);
            painter.drawText(xPos + 5, yPos, colWidths[col] - 10, rowHeight,
    ...
    // Ajouter un résumé en bas de la dernière page
    if (totalVehicules > 0) {
        ...
        yPos += 20;
        painter.setFont(headerFont);
        QString resume = QString("Total des véhicules exportés : %1").arg(totalVehicules);
        painter.drawText(margin, yPos, pageWidth - 2 * margin, 30, Qt::AlignRight | Qt::AlignVCenter, resume);
    }

    painter.end();

    if (totalVehicules == 0) {
        QMessageBox::warning(this, "Avertissement", "Aucun véhicule à exporter.\nLe fichier PDF a été créé mais il est vide.");
    } else {
        QMessageBox::information(this, "Succès", QString("Export PDF réussi!\n%1 véhicule(s) exporté(s).\nLe fichier a été enregistré sous :\n%2").arg(totalVehicules).arg(fileName));
    }
}
...
void Vehicule::resizeEvent(QResizeEvent *event)
{
    ...
    // Repositionner la notification en bas à droite si elle existe
...
void Vehicule::on_QR_CODE_2_clicked()
{
    ...
    // Générer le contenu du QR code avec une URL
    QString dataUrl = genererTexteQRCode(matricule);

    // Vérifier que le contenu a été généré correctement
    if (dataUrl.startsWith("Erreur:")) {
        QMessageBox::critical(this, "Erreur", dataUrl);
        return;
    }

    // Générer l'image du QR code
    QImage qrImage = ::QRCodeGenerator::generateQRCode(dataUrl, 400);
    ...
    // Sous-titre
    QLabel *subtitleLabel = new QLabel("Scannez ce QR code avec votre appareil mobile", dialog);
    subtitleLabel->setStyleSheet("color: #7f8c8d; margin-bottom: 15px;");
    ...
    qrLayout->setContentsMargins(15, 15, 15, 15);
    ...
    QLabel *infoLabel = new QLabel(
        ...
        "Ce QR code contient toutes les informations du véhicule.<br>"
        "Scannez-le avec l'application photo de votre smartphone."
        "</div>",
        dialog
        );
    ...
    qrLayout->addWidget(infoLabel);
    qrLayout->addWidget(closeButton, 0, Qt::AlignHCenter);

    mainLayout->addWidget(headerLabel);
    mainLayout->addWidget(subtitleLabel);
    mainLayout->addWidget(qrFrame, 1);
...
QString Vehicule::genererTexteQRCode(const QString& matricule)
{
    ...
    QString fileUrl = genererFichierHTML(matriculeValue, modele, kilometrage, disponibilite, typeEnergie, etatMaintenance, dateMaintenance, prediction);
    if (fileUrl.startsWith("Erreur:")) {
        return fileUrl;
    }
    ...
    QString ip = selectBestLanIp();
    if (ip.isEmpty()) {
        ip = QHostAddress(QHostAddress::LocalHost).toString();
    }

    QString infoUrl;
    if (httpServerAlt && httpServerAlt->isListening()) {
        infoUrl = QString("http://%1/%2").arg(ip).arg(fileName);
    } else {
        infoUrl = QString("http://%1:%2/%3").arg(ip).arg(httpPort).arg(fileName);
    }
    return infoUrl;
}
