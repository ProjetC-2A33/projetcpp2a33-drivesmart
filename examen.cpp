...
Examen::Examen(QString id_examen, QString type_examen, QString type_permis, QDate date_examen) :
    QWidget(nullptr),
    ui(new Ui::Examen)
{
    ui->setupUi(this);
...
QSqlQueryModel* Examen::afficher()
{
    ...
    model->setQuery("SELECT ID_EXAMEN, TYPE_EXAMEN, TYPE_PERMIS, DATE_EXAMEN FROM EXAMEN");
    return model;
}
...
{
    if (row < 0 || row >= ui->tab_5->rowCount()) return;
    auto getText = [&](int col){ QTableWidgetItem *it = ui->tab_5->item(row, col); return it ? it->text() : QString(); };
    ...
    if (!d.isValid()) d = QDate::fromString(getText(3), "dd/MM/yyyy");
    if (d.isValid()) ui->dateEdit_5->setDate(d);
}
